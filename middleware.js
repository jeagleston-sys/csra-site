...
export default async function middleware(req) {
  ...
  // Unlock: POST /view/_auth  {"job":"<project>","code":"<access code>"}
  if (path === '/view/_auth') {
    if (req.method !== 'POST') return json(405, { error: 'method' });
    let body;
    try { body = await req.json(); } catch { return json(400, { error: 'bad-request' }); }
    const slug = String(body.job || '').toLowerCase();
    const expected = SLUG.test(slug) && Object.prototype.hasOwnProperty.call(codes, slug) ? normalize(codes[slug]) : null;
    const given = normalize(body.code);
    // Unknown project and wrong code look identical from outside
    if (!expected || !given || !safeEqual(given, expected)) return json(401, { error: 'bad-code' });
